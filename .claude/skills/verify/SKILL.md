...
dirty-Pull failure scenarios prove the inverse race: `pull_push` delivers its
failure through `set_app_notice`; Remote Browse and an earlier plain AppNotice
stay open, queued notices retain arrival order across render-time drains, and a
delayed Acknowledge still executes.
