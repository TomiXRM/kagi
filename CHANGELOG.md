...
- Remote Browse and Update now occupy the same modal slot as repository confirmations and app notices. Workspace and Welcome apply one shared modal-key routing wrapper, so Enter and Escape act on the same slot regardless of which surface is visible and cannot fall through to a selected commit or diff. Independently-rendered modal stacks are no longer possible. Replacing the slot requeues actionable reconciliation notices instead of discarding their Inspect/Acknowledge path; asynchronously arriving notices always wait behind any occupied slot—including an earlier plain notice—without disturbing FIFO order. The running update installer remains window-owned when its modal closes, preventing duplicate installs and retaining completion status for the next open. (#643, #718, ADR-0197 S3a)
