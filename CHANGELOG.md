...
- **Stash & Pull names the paths whose restore would conflict, before you confirm.** A dirty Pull now fetches first and its confirmation lists the paths that are modified locally and also changed by the incoming update, instead of only counting the changes and discovering the collision when the stash failed to restore. A fast-forward pull cannot conflict commit-to-commit, which is why the existing merge prediction stayed silent for exactly this case. A failed fetch opens no confirmation and reports itself in the footer. (#625, ADR-0192)
