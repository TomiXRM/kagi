...
- Remote Browse and Update now occupy the same modal slot as repository confirmations and app notices. Enter and Escape can no longer fall through to a selected commit or diff while either modal is open, independently-rendered modal stacks are no longer possible, and Escape still closes the slot on Welcome. The running update installer remains window-owned when its modal closes, preventing duplicate installs and retaining completion status for the next open. (#643, #718, ADR-0197 S3a)
