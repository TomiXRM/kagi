...
> **⬇️ ダウンロード:** **macOS / Linux / Windows** 向けのビルド済みアプリを [**Releases ページ**](https://github.com/TomiXRM/kagi/releases) で配布しています(下の[インストール](#-インストール)参照)。v0.3.4 以降はアプリ内から更新の確認・インストールもできます。
...
- ⚔️ **conflict 解決** — merge / rebase / cherry-pick / revert で conflict したら **Conflict Mode** に入り、GitKraken 風の 3 ペインエディタで file / chunk / **行単位**の採用チェック、リアルタイム Result プレビュー/編集、conflict ダッシュボード、Save→stage / Continue フローを提供。中止すれば操作前の状態に復元できます
...
- 🌐 **英語 / 日本語 UI** — 説明文は翻訳しつつ、Git のドメインワード(Commit, Push, Pull, merge, conflict, branch…)は両言語とも英語のまま
- 🔍 **UI 全体ズーム** — ヘッダ・パネル・モーダル・インスペクタ・メニュー・コミットグラフが一斉にスケール
...
<div align="center">
<img src="docs/images/conflict.png" width="850" alt="Conflict Mode — GitKraken 風の 3 ペインエディタ。file/chunk/行単位の採用、リアルタイム Result プレビュー、conflict ダッシュボード" />
</div>
...
| conflict は罠ではなく復旧可能 | Conflict Mode への突入は常に巻き戻せる — 中止すれば操作前の状態に正確に復元し、解決途中の内容は自動保存される |
...
| Windows(x86_64)| `kagi-<version>-x86_64-windows.zip` — 展開して `kagi.exe` を実行(アセット同梱の単体実行ファイル) |

AppImage の場合: `unzip kagi_Linux-AppImage_<arch>.zip && bash install_linux_desktop.sh` で `~/.local` 配下に登録(アイコン + `.desktop`、ネットワーク不要)。
...
必要なもの: Rust stable(rustup)、加えて:

- **macOS** — **Xcode Command Line Tools のみ**(full Xcode 不要。GPUI の `runtime_shaders` を使用)。
- **Linux** — GPUI のネイティブビルド依存。Debian/Ubuntu の場合:
  ...
  (これらが無いと `cargo run` がビルドに失敗します。特に `libxkbcommon-x11-dev` の不足がよくあります。)
...
活発に開発中。実装済み: コミットグラフ UX 一式、branch / tag / stash / worktree 管理、ステージング + コミットスイート、dry-run 安全機構付きの cherry-pick / revert / amend / discard、**conflict 解決(Conflict Mode + 行単位 3 ペインエディタ + merge-into-conflict)**、リポジトリタブ、テーマ、**英語 / 日本語 UI**、UI 全体ズーム、内蔵ターミナル、GitHub アバター、配布パイプライン。ロードマップは [docs/requirements.md](docs/requirements.md) とチケット INDEX を参照。
