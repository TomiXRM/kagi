...
各リリースにはプラットフォームごとのビルド済みバイナリが付属するため(ADR-0120)、普段使っているツールでインストールできます。

```sh
# Homebrew(macOS Apple Silicon / Linux)— `kagi` CLI をインストール
brew install https://github.com/TomiXRM/kagi/releases/latest/download/kagi.rb

# cargo binstall(crates.io 未公開のためリポジトリから取得)
...
インストール後は `kagi --version` で確認できます。(raw URL 経由の Homebrew は単発インストールです。`brew upgrade` 用の `homebrew-kagi` tap は今後対応予定で、それまではアプリ内アップデータが更新を担います。)
...
パッケージマネージャはリリースに付属する `kagi-<version>-<target-triple>.{tar.gz,zip}`(素のバイナリ)を利用します。
