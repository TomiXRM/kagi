...
| Linux(x86_64 / arm64)| `kagi-<version>-x86_64.tar.gz`(バイナリ + `.desktop` + アイコン)、または AppImage zip `kagi_Linux-AppImage_<arch>.zip` |
...
タグ `v*` を push すると [release workflow](.github/workflows/release.yml) が macOS arm64 / x86_64 + Linux x86_64 / arm64 をビルド(tar.gz + AppImage zip)し、checksum 付きで draft release に上げます。
...
[Apache License 2.0](LICENSE-APACHE) または [MIT License](LICENSE-MIT) のデュアルライセンスです(どちらかを選択)。
vendored の `vendor/gpui-terminal` も同じく MIT OR Apache-2.0 です。
