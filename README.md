...
| Linux (x86_64 / arm64) | `kagi-<version>-x86_64.tar.gz` (binary + `.desktop` + icon), or the AppImage zip `kagi_Linux-AppImage_<arch>.zip` |
...
Tagging `v*` runs the [release workflow](.github/workflows/release.yml): macOS arm64 + x86_64 + Linux x86_64/arm64 builds (tar.gz + AppImage zip), checksums, and a draft GitHub release.
