...
| Path | What |
| --- | --- |
| `icon-512x512.png` | **Source** app-icon image (square PNG with alpha). Edit this to change the icon. The `512x512` in the name is historical — any square size works, the pipeline normalises to 1024². |
...
- `icon.ico` — Windows exe icon, embedded into `kagi.exe` by `build.rs`.
...
### How it works
- The `.ico` packs PNG-compressed entries at 16/32/48/64/128/256 (Vista+),
  written by a stdlib `python3` step — no Pillow or ImageMagick. It used to be
  a hand-made file outside this pipeline, which meant a source swap silently
  left Windows on the old icon.
