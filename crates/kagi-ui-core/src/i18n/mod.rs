...
impl Msg {
    ...
    pub fn t(self) -> &'static str {
        ...
        match (lang(), self) {
            ...
            (En, IssueWrite) => "Write",
            (Ja, IssueWrite) => "編集",
            (En, IssuePreview) => "Preview",
            (Ja, IssuePreview) => "プレビュー",
            (En, IssueFocusEditor) => "Focus Editor",
            (Ja, IssueFocusEditor) => "エディタを拡大",
            (En, IssueExitFocus) => "Exit Focus Editor",
            (Ja, IssueExitFocus) => "通常表示に戻る",
            (En, IssueDraftSaved) => "Draft saved",
            (Ja, IssueDraftSaved) => "下書き保存済み",
            (En, IssueDraftSaving) => "Saving draft…",
            (Ja, IssueDraftSaving) => "下書き保存中…",
