...
use gpui::{font, hsla, rgb, App, Font, FontFallbacks, Hsla};
...
/// Bundled Japanese fallback. Inter intentionally has no CJK glyphs, so this
/// family must be attached explicitly to the window root instead of leaving
/// Han-unification to Linux's installed-font set and locale configuration.
pub const CJK_FONT: &str = "Noto Sans JP";

/// The deterministic UI font stack used by the gpui-component window root.
///
/// A fresh value is cheap (`FontFallbacks` stores its vector behind an `Arc`)
/// and keeps all overlays, async child entities, and ordinary Kagi elements on
/// the same Inter -> Noto Sans JP resolution path.
pub fn ui_font() -> Font {
...
mod tests {
    ...
    fn linux_cosmic_text_uses_bundled_fallback_for_regular_and_bold() {
        ...
        use gpui::{px, FontWeight, TextRun, TextSystem};
        ...
        for weight in [FontWeight::NORMAL, FontWeight::BOLD] {
            ...
            let layout = text_system.layout_line(text, px(16.0), &[run], None);
            assert_eq!(
                layout.runs.len(),
                2,
