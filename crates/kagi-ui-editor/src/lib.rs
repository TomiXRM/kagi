...
//! shape as `FileHistoryView`): it holds `repo_path` and drives its own file
//! reads off `cx.background_spawn`, so entity-initiated actions (tree click →
//! select) update *self* and never re-enter `KagiApp` (which would
//! double-borrow the leased entity and panic). Everything outward — close,
//! dirty guard, toasts, and the two `Backend` reads (ADR-0121 C4) — is an
//! [`EditorWorkspaceEvent`] the bin subscribes to; there is no `KagiApp`
//! back-reference.
//!
...
use gpui::prelude::*;
use gpui::{
    div, px, rgb, uniform_list, AnyElement, Context, Entity, EventEmitter, MouseButton,
...
/// these events; the bin subscribes in `open_editor_workspace` and maps them
/// onto `KagiApp` methods. `FilesRequested`/`DiffRequested` are the two
/// Git-backend reads — the bin runs them and seeds the result back
...
pub enum EditorWorkspaceEvent {
    ...
    /// (`ActiveModal` lives on `KagiApp`, ADR-0076/0093).
    DirtyGuardRequested(EditorGuardIntent),
    /// A save was refused because the file changed on disk → host toasts.
    SaveBlocked,
    /// The background save write failed → host toasts + footer.
    SaveFailed { path: PathBuf, error: String },
    /// Load the working-tree file list for `source` (needs `Backend`); seed
    /// the result via `seed_files(generation, …)`.
    FilesRequested { generation: u64, source: TreeSource },
...
/// The crate-side subset of the bin's `EditorPendingIntent`: the guard
/// intents this view itself can originate. The bin's enum keeps its extra
/// repo-context variants (SwitchRepo / CloseRepoTab / EnterRemoteView) and
/// maps these onto itself.
#[derive(Clone, Debug)]
pub enum EditorGuardIntent {
    /// Discard the buffer and re-read the open file from disk.
    Reload,
...
/// Host-supplied hooks (ADR-0121 C4): the bin-owned pure helpers this crate
/// renders/loads with but must not own — `lang_for_path` (diff_view's
/// filename+extension table), `image_from_bytes` (avatar_fetch's sniffing
/// decoder), and the hunks-pane diff renderer (`render_helpers::
/// render_diff_list` over the bin's `MainDiffView`, which also owns the
/// downcast of [`EditorWorkspaceView::diff`]).
...
pub struct EditorWorkspaceView {
    ...
    content_sig: u64,
    /// The selected file's WIP diff, seeded by the bin ([`seed_diff`]
    /// (Self::seed_diff)) as an opaque `MainDiffView` (a bin type this crate
    /// can't name — `hooks.render_hunks` downcasts it back). `None` while
...
impl EditorWorkspaceView {
    ...
    /// Kick off the file-list load for the current `source` — emits
    /// [`EditorWorkspaceEvent::FilesRequested`]; the bin runs the
    /// `Backend` reads off-thread and marshals the result back into *this*
    /// entity via [`Self::seed_files`], guarded by `generation` so a
    /// superseded reload no-ops (and a dropped entity —
    /// `close_editor_workspace` — simply no-ops too). Bumping `generation`
    /// on every call also makes a `TreeSource` switch mid-load safe: the
    /// stale load's result is dropped.
    pub fn start_load(&mut self, cx: &mut Context<Self>) {
        self.loading = true;
        self.error = None;
        self.generation = self.generation.wrapping_add(1);
        cx.emit(EditorWorkspaceEvent::FilesRequested {
            generation: self.generation,
            source: self.source,
        });
    }

    /// The bin's answer to [`EditorWorkspaceEvent::FilesRequested`]: the
    /// merged file list (or the load error) for `generation`. Body is the
    /// former in-crate marshal-back, unchanged.
    pub fn seed_files(
        &mut self,
        generation: u64,
        ...
        match result {
            Ok(files) => {
                self.tree = build_workspace_tree(&files);
                ...
                self.collapsed = match self.source {
                ...
                if generation == 1 && self.source == TreeSource::Changes && was_empty {
                    cx.notify();
                    self.switch_source(TreeSource::All, cx);
                    return;
                }
                ...
                let (sel, load) =
                    restore_selection(self.open_path.is_some(), restore, self.files.len());
                match (sel, load) {
                    (Some(i), true) => self.select(i, cx),
                    (sel, _) => self.selected = sel,
                }
            }
    ...
    pub fn request_close_tab(&mut self, path: PathBuf, cx: &mut Context<Self>) {
        if self.tab_dirty(&path) {
            self.open_dirty_guard(EditorGuardIntent::CloseTab(path), cx);
            return;
        }
    ...
    /// Ask the host to open the unsaved-changes confirmation (`ActiveModal`
    /// lives on `KagiApp`, ADR-0076/0093). Emitted as an event — `cx.emit`
    /// queues a deferred effect, so the arrow-key path
    /// (`KagiApp::step_editor_ws_selection`, which calls into this entity
    /// from within an already-leased `KagiApp` listener) can't double-lease
    /// the app the way a synchronous back-call would.
    fn open_dirty_guard(&self, intent: EditorGuardIntent, cx: &mut Context<Self>) {
        cx.emit(EditorWorkspaceEvent::DirtyGuardRequested(intent));
    }
    ...
    fn load_selected(&mut self, cx: &mut Context<Self>) {
        ...
        let image_from_bytes = self.hooks.image_from_bytes;
        // The WIP diff needs `Backend` — ask the host; it lands via
        // `seed_diff`, guarded by the same `file_req`.
        cx.emit(EditorWorkspaceEvent::DiffRequested {
            req: file_req,
            path: path.clone(),
        });
        ...
        let task = cx.background_spawn(async move {
            ...
            (text, is_binary, image, missing, undecodable, too_large)
        });

        cx.spawn(async move |view, acx| {
            let (text, is_binary, image, missing, undecodable, too_large) = task.await;
            let _ = view.update(acx, |v, cx| {
                ...
                v.content_lang = (v.hooks.lang_for_path)(&path).unwrap_or("text");
    ...
    /// Ask the host to close this view (drops the entity). Guarded by
    /// the unsaved-changes modal when ANY tab is dirty (T-WS-EDITOR-002 §5;
    /// backgrounded tabs count — dropping the entity loses them all).
    fn request_close(&self, cx: &mut Context<Self>) {
        if self.any_dirty() {
            self.open_dirty_guard(EditorGuardIntent::Close, cx);
            return;
        }
        cx.emit(EditorWorkspaceEvent::CloseRequested);
    }
    ...
    /// host toasts), and leave the decision to the external-change banner
    /// (Overwrite / Reload).
...
pub fn dir_path_for_tree_index(tree: &[TreeRow], index: usize) -> Option<PathBuf> {
...
pub fn merge_working_tree_files(status: &WorkingTreeStatus) -> Vec<WorkspaceFile> {
...
/// T-WS-EDITOR-007).
pub fn merge_all_files(all_paths: Vec<PathBuf>, changed: &[WorkspaceFile]) -> Vec<WorkspaceFile> {
...
fn render_center_pane(
    ...
    if view.external_changed {
        let reload = cx.listener(|this, _e: &gpui::ClickEvent, _w, cx| {
            this.open_dirty_guard(EditorGuardIntent::Reload, cx);
        });
...
// ── Local copies of tiny bin helpers (ADR-0121 C4) ────────────────────
// Duplicated deliberately, C2-precedent: importing them would drag in the
// bin (impossible) or force moving contended shared modules
// (`render_helpers.rs` / `mod.rs`) mid-C3.

/// Cheap FNV-1a `(path, content)` signature for the sig-guarded editor push
/// (same algorithm as the bin's `conflict_content_sig`, preview arm).
fn buffer_sig(path: &Path, result: &str) -> u64 {
...
/// T-DIFF-WRAP-001: construct a fresh diff-list [`gpui::ListState`] (item
/// count 0 — the bin's `render_diff_list` syncs it). `px(1000.)` overdraw
/// matches the bin's `render_helpers::new_diff_list_state`.
fn new_diff_list_state() -> gpui::ListState {
    gpui::ListState::new(0, gpui::ListAlignment::Top, px(1000.))
}

/// Vertical-scrollbar wrapper (same shape as the bin's
/// `render_helpers::with_vertical_scrollbar`).
fn with_vertical_scrollbar<H: gpui_component::scroll::ScrollbarHandle + Clone>(
    id: &'static str,
    handle: &H,
...
mod tests {
    ...
    fn merge_working_tree_files_flags_untracked_staged_unstaged() {
        let status = WorkingTreeStatus {
            staged: vec![kagi_domain::status::FileStatus {
                path: PathBuf::from("staged.rs"),
                change: ChangeKind::Modified,
            }],
            unstaged: vec![kagi_domain::status::FileStatus {
