...
- `ci.yml` の `blocking-ci.needs` を必須 job 集合の唯一の定義とする。
  invariant matrix 全体、gates の lint/type/selftest/fixture、macOS test、Windows build
  (2026-09-23 #728 で追加) を依存に持つ。
...
- Linux / fmt-clippy の advisory job は依存集合に含めず、従来通り非 blocking。
  Windows は当初 advisory だったが、job object の stop proof（`proc::job::`）が
  そこでしか走らないため **2026-09-23 (#728) に blocking へ移した**: `build (Windows)`
  は `continue-on-error` を外して `blocking-ci.needs` に入る（ADR-0196 決定 5 / Wave 3）。
