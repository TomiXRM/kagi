# T-BCM-010: Copy branch name を実装する
...
local は短名、remote は origin/x 形式。toast「Copied <name>」(W18 と同型)
...
- `Copy branch name` writes the selected local short name or remote `origin/x` display name to the clipboard.
- completion toast is `Copied <name>`.
