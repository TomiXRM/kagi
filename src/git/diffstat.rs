...
use super::{ChangeKind, CommitId, GitError, resolve_head, Head};
...
// ────────────────────────────────────────────────────────────
// Bar segment computation (T-DIFFSTAT-003)
// ────────────────────────────────────────────────────────────
...
pub fn bar_segments(additions: usize, deletions: usize, max_segments: usize) -> (usize, usize) {
    ...
    if max_segments == 1 {
        ...
        return if additions >= deletions { (1, 0) } else { (0, 1) };
    }
        ...
        for a in 0..50 {
            for d in 0..50 {
                for m in 1..=8 {
                    ...
                    if a + d > 0 {
                        if a > 0 && m >= 2 && d > 0 {
                            assert!(g >= 1, "green must be >=1 when additions exist ({a},{d},{m})");
