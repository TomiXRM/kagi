...
//!
//! The resolution buffer holds per-file **Result drafts** for an in-progress
//! conflict, operating entirely **in memory** — the working tree and index are
//! never touched here (only [`crate::git::conflicts::execute_conflict_continue`]
//! writes them, and only at execute time).  This is kagi's in-memory-first
//! principle applied to conflict resolution (requirements §D).
//!
//! # What it does
//!
//! - **Materialization** (zdiff3): a file's initial draft is built from
//!   git2's `merge_file_from_index` with `style_zdiff3` so the Base context is
//!   visible inside the markers (ADR-0058 §marker style).  If zdiff3 is
//!   unavailable or the file has no usable text merge (binary / modify-delete)
//!   the buffer falls back gracefully (standard markers, or no draft).
//! - **Choices**: [`ResolutionBuffer::apply_choice`] sets a file's Result to
//!   the current side, incoming side, or both sides in a chosen order
//!   ([`ResolutionChoice`]).  [`ResolutionBuffer::set_manual_text`] replaces the
//!   Result with hand-edited text.  Both push onto a per-file undo stack.
//! - **Undo / redo**: per-file history ([`ResolutionBuffer::undo`] /
//!   [`ResolutionBuffer::redo`]).
//! - **Provenance**: each Result line records its origin
//!   ([`LineOrigin::Current`] / `Incoming` / `Manual`) for the UI's
//!   out-of-source visualization (BC / KDiff3 style).
//! - **Autosave / load**: hand-written JSON (no `serde`, like `drafts.rs`) under
//!   `~/.kagi/conflicts/<sha1(repo)>/` so a partial resolution survives a
//!   restart / abort and can be reloaded.
//! - **Marker-residue gate**: [`ResolutionBuffer::files_with_marker_residue`]
//!   reuses [`crate::git::checklist::text_has_conflict_marker`] so the continue
//!   gate and the checklist agree.
//!
//! Hard rules honored: `chars()`-only on user text (no byte slicing of content);
//! line splitting is on `\n` over `&str`; no force ops; in-memory only.
...
impl ConflictHunk {
    ...
    pub fn side_state(&self, side: SelectionSide) -> TriState {
        ...
        if all { TriState::All } else { TriState::None }
    }
...
impl HunkModel {
    ...
    fn with_hunk_mut(
        &mut self,
        hunk_index: usize,
        mut f: impl FnMut(&mut ConflictHunk),
    ) -> bool {
        let mut i = 0usize;
        for r in &mut self.regions {
            if let Region::Hunk(h) = r {
    ...
    pub fn set_hunk_line(
        ...
        self.with_hunk_mut(hunk_index, |h| {
            changed = h
                .ensure_line_selection()
                .set_line(side, line_index, taken);
        }) && changed
    }
...
impl ResolutionBuffer {
    ...
    pub fn apply_choice(
        ...
        let fr = self
            .files
            .get_mut(path)
            .ok_or_else(|| GitError::Other(format!("not a tracked conflict: {}", path.display())))?;
...
impl ResolutionBuffer {
    ...
    pub fn autosave(&self) -> Result<PathBuf, GitError> {
        let path = buffer_file_path(&self.repo_path).ok_or_else(|| {
            GitError::Other("conflicts: could not determine autosave dir (no HOME or KAGI_LOG_DIR)".to_string())
        })?;
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                GitError::Other(format!("conflicts: mkdir failed for {}: {}", parent.display(), e))
            })?;
        }
        let json = self.to_json();
        std::fs::write(&path, json.as_bytes()).map_err(|e| {
            GitError::Other(format!("conflicts: write failed for {}: {}", path.display(), e))
        })?;
        Ok(path)
    }
...
fn text_to_lines(text: &str, origin: LineOrigin) -> Vec<ResolvedLine> {
    if text.is_empty() {
        return Vec::new();
    }
    let mut lines: Vec<&str> = text.split('\n').collect();
...
fn lines_to_text(lines: &[ResolvedLine]) -> String {
    if lines.is_empty() {
        return String::new();
    }
...
mod tests {
    ...
    fn choice_both_orders() {
        ...
        b.apply_choice(&p, ResolutionChoice::BothCurrentFirst).unwrap();
        ...
        b.apply_choice(&p, ResolutionChoice::BothIncomingFirst).unwrap();
    ...
    fn json_round_trips_result_and_provenance() {
        ...
        assert_eq!(
            ...
            vec![LineOrigin::Current, LineOrigin::Current, LineOrigin::Incoming]
        );
        // Side texts survive.
