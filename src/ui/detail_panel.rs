//! Commit detail panel — T010 / T019 / T022
//!
...
//!
//! T022: soft_wrap / ZWSP scheme removed.  Fields no longer contain U+200B.
//! Use `truncate()` in the render layer for single-line fields, and split
...
///
/// T022: All fields are **raw** strings — no ZWSP.  Single-line fields
/// (SHA, author, committer) are truncated in the render layer via
/// `truncate()`.  The message is stored as raw multi-line text and split
/// on `'\n'` at render time so that each line can be truncated independently.
#[derive(Clone)]
pub struct CommitDetail {
    /// Full 40-hex SHA (raw).
    pub full_sha: SharedString,
    /// `name <email>  YYYY-MM-DD HH:MM` — raw, no ZWSP.
    ...
    /// Capped at 2000 chars to avoid extreme panel heights.
...
fn commit_to_detail(c: &Commit) -> CommitDetail {
    ...
    // Author: `name  <email>  YYYY-MM-DD HH:MM` — raw, no ZWSP.
...
mod tests {
    ...
    fn no_zwsp_in_author_line() {
        ...
        let c = Commit {
            ...
            author: Signature {
                name: "Some Very Long Name That Used To Trigger ZWSP Wrapping".to_string(),
                email: "some.very.long.email.address@example.domain.com".to_string(),
                time: 0,
            },
            committer: Signature {
                name: "Some Very Long Name That Used To Trigger ZWSP Wrapping".to_string(),
        ...
        assert!(
            !detail.author_line.contains('\u{200B}'),
            "author_line must not contain ZWSP"
        );
        assert!(
            !detail.full_sha.contains('\u{200B}'),
            "full_sha must not contain ZWSP"
        );
        assert!(
            !detail.full_message.contains('\u{200B}'),
            "full_message must not contain ZWSP"
        );
    }
    ...
    fn message_preserves_newlines() {
        ...
        let detail = commit_to_detail(&c);
        // The raw message should be preserved (with trailing whitespace trimmed).
        let stored = detail.full_message.as_ref();
        // Must contain the two blank-line separators.
