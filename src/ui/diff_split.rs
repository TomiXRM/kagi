...
/// Shared split projections for all diff embeddings. The existing cache now
/// retains pairing and move detection together, keyed by weak row ownership
/// rather than selection identity. Rendering is main-thread only.
...
/// (budget-capped) greedy match — call through [`split_projection`] from
