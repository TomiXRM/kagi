...
impl KagiApp {
    ...
    pub fn detect_conflict_mode(&mut self) {
        ...
        // Preserve the previously-open editor file / view toggle across
        // re-detections (W33: the dashboard owns these; W32 reads editing_file).
    ...
    /// Set the "currently-editing conflict file" state (W33 → W32 handoff): the
    /// W32 Conflict Editor lane reads `conflict.editing_file` to open its editor.
    /// This lane does NOT implement the editor.
    pub fn conflict_open_editor(&mut self, idx: usize) {
        if let Some(c) = self.conflict.as_mut() {
            if idx < c.session.files.len() {
