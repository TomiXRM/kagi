...
impl WorkspaceItem for FileHistoryItem {
    ...
    fn render(
        ...
        ev.update(cx, |v, cx| {
            if !Arc::ptr_eq(&v.avatars, avatars) {
                v.avatars = Arc::clone(avatars);
                cx.notify();
            }
        });
...
impl WorkspaceItem for EditorWorkspaceItem {
    ...
    fn render(
        ...
        // Keep the latest shared snapshot even while Diff is selected: the
        // child's switch to History need not re-render its parent.
        let avatars = &app.avatars.images;
        ev.update(cx, |v, cx| {
