...
fn five_ops_produce_five_jsonl_lines() {
    ...
    let ops = [
        (
            "checkout",
            OpOutcome::Success {
                after: make_summary("branch: feature", "clean"),
            },
        ),
        (
            "create-branch",
            OpOutcome::Success {
                after: make_summary("branch: main", "clean"),
            },
        ),
        (
            "stash-push",
        ...
        (
            "stash-apply",
            OpOutcome::Success {
                after: make_summary("branch: main", "2 modified"),
            },
        ),
        (
            "cherry-pick",
...
fn refused_entry_recorded() {
    ...
    assert!(
        line.contains("Working tree has 1 modified"),
        "blocker 1 missing"
    );
...
fn failed_entry_recorded() {
    ...
    let entry = make_entry(
        ...
        99,
        OpOutcome::Failed {
            error: "stash push failed: nothing to stash".to_string(),
        },
    );
...
fn read_tail_empty_when_no_file() {
    ...
    assert!(
        entries.is_empty(),
        "expected empty vec when no file, got {} entries",
        entries.len()
    );
...
fn read_tail_round_trips_all_outcome_kinds() {
    ...
    let entries_to_write = vec![
        ...
        make_entry(
            ...
            1002,
            OpOutcome::Failed {
                error: "nothing to stash".to_string(),
            },
        ),
        make_entry(
            "stash-apply",
            1003,
            OpOutcome::Refused {
                blockers: vec!["dirty tree".to_string(), "conflict".to_string()],
            },
        ),
    ];
...
fn read_tail_skips_malformed_lines() {
    ...
    let valid2 = make_entry(
        "create-branch",
        9001,
        OpOutcome::Success {
            after: make_summary("branch: feat", "clean"),
        },
    );
...
fn read_tail_restores_escaped_strings() {
    ...
    let e = &read[0];
    assert_eq!(
        e.repo, "/path/with \"quotes\" and \\backslash",
        "repo roundtrip"
    );
    assert_eq!(
        e.before.dirty, "1 modified\nfile",
        "before.dirty roundtrip with embedded newline"
    );
    match &e.outcome {
        OpOutcome::Failed { error } => {
